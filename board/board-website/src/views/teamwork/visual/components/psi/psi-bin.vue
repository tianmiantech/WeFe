<!-- psi分箱配置组件 -->
<template>
    <el-form>        <div class="psi-title">{{title}}</div>
        <el-form-item label="分箱方式">
            <el-select :model-value="binValue.method" @change="selectChange" :disabled="disabled">
                <el-option
        ...
        <el-form-item label="分割点" v-if="binValue.method == 'custom'">
            <el-input 
                ...
                placeholder="请输入分割点"
                @input="splitChange"></el-input>
            <p class="f12 pl10 pr10 color-danger">请填写分割点，用逗号隔开,如0-1之间等宽分五箱，请填入0,0.2,0.4,0.6,0.8,1</p></el-form-item>
        <!-- </div> -->
    </el-form>            
...
<script setup>
    ...
    const options = ref(baseOptions.filter(item => !props.filterMethod.includes(item.value)));
