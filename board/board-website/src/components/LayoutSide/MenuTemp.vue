<template>
    <ul class="sub-menu-list">
        <template v-for="(item, index) in menus">
            <template
                v-if="!item.meta.asmenu && item.children && !item.meta.hidden"
            >
                <el-sub-menu
                    :key="item.name"
                    :index="`${item.path}`"
                        ...
                        <span
                            ...
                            >
                                <template #content>
                                    <p
                                        class="color-danger"
                                        v-html="item.meta.globalTooltip"
                                    ></p>
                                </template>
                                <el-icon class="ml5">
                                    <elicon-info-filled />
            ...
            <template
                v-else-if="
                    item.meta.asmenu && !item.meta.hidden && item.children
                "
            >
                <el-menu-item :key="index" :index="item.children[0].path">
                    <el-icon class="icon">
                        <component
                            :is="`elicon-${item.children[0].meta.icon}`"
                        />
                    </el-icon>
                    <template #title>
