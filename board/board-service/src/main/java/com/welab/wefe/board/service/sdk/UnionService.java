...
public class UnionService extends AbstractUnionService {
     ...
     * 获取数据资源详情
     */
    public <OUT> OUT getDataResourceDetail(String dataResourceId, DataResourceType type, Class<OUT> outputClass) throws StatusCodeWithException {
        String key = dataResourceId + "getDataResourceDetail";
        if (CACHE_MAP.containsKey(key)) {
            return (OUT) CACHE_MAP.get(key);
        }

        JObject params = JObject.create("data_resource_id", dataResourceId);
        JSONObject result = request(StringUtil.stringToUnderLineLowerCase(type.name()) + "/detail", params);
