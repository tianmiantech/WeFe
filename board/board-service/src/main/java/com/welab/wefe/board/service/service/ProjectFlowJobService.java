...
public class ProjectFlowJobService extends AbstractService {
    ...
    public synchronized String startFlow(StartFlowApi.Input input) throws StatusCodeWithException {
		...
		// 这个判断不知道有什么意义，所以先隐藏掉
		// long memberCount = jobMembers.stream().filter(x -> x.getJobRole() !=
		// JobMemberRole.arbiter).count();
		// if (memberCount < MIX_FLOW_PROMOTER_NUM && !isOotMode) {
		// throw new StatusCodeWithException("需要在【" + ComponentType.DataIO.getLabel() +
		// "】中选择两个或两个以上的数据集", StatusCode.PARAMETER_VALUE_INVALID);
		// }
