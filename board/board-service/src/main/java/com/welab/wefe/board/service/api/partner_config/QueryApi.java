 ...
 * @date 2022/9/8
 */
@Api(path = "partner_config/query", name = "分页查询合作方配置记录")
public class QueryApi extends AbstractApi<QueryApi.Input, PagingOutput<PartnerConfigOutputModel>> {

    @Autowired
    private PartnerConfigService partnerConfigService;

    @Override
    protected ApiResult<PagingOutput<PartnerConfigOutputModel>> handle(Input input) throws Exception {
        return success(partnerConfigService.query(input));
    }
