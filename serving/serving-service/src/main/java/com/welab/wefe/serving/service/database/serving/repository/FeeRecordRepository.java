...
public interface FeeRecordRepository extends BaseRepository<FeeDetailOutputModel, String> {
     ...
     * @return
     */
    @Query(value = "select concat(fd.service_id, fd.client_id,fd.fee_config_id,max(fd.created_time) as created_time) as id, max(fd.service_name) as service_name,fd.service_id as service_id, max(fd.client_name) as client_name, " +
