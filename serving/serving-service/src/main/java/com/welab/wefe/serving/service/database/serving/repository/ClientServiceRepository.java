...
//            "(SELECT cs.id , s.name as serviceName, c.name as clientName, cs.status , cs.created_time, s.service_type , c.ip_add ,s.url " +
//            "FROM wefe_serving.client_service cs " +
//            "left join service s on cs.service_id = s.id " +
//            "left join client c on cs.client_id = c.id ) as t " +
//            "where if(:service_name !='' ,t.serviceName = :service_name, 1=1) and " +
//            "      if(:client_name !='' ,t.clientName = :client_name, 1=1) and " +
//            "      if(:status !='' ,t.status = :status, 1=1) ", nativeQuery = true)
