...
public class DetailApi extends AbstractApi<ApiDataResourceDetailInput, ApiDataResourceQueryOutput> {
    ...
    protected ApiResult<ApiDataResourceQueryOutput> handle(ApiDataResourceDetailInput input) throws StatusCodeWithException {
        ...
        switch (input.getDataResourceType()) {
            case BloomFilter:
                dataResourceQueryOutput = dataResourceMongoReop.findCurMemberCanSee(input.getDataResourceId(), input.getCurMemberId(), MongodbTable.Union.BLOOM_FILTER);
                break;
            case TableDataSet:
                dataResourceQueryOutput = dataResourceMongoReop.findCurMemberCanSee(input.getDataResourceId(), input.getCurMemberId(), MongodbTable.Union.TABLE_DATASET);
                break;
            case ImageDataSet:
