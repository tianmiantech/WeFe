...
from common.python.common.exception.custom_exception import DataSetEmptyError
...
from kernel.protobuf.generated.data_io_meta_pb2 import DataIOMeta
from kernel.protobuf.generated.data_io_meta_pb2 import ImputerMeta
from kernel.protobuf.generated.data_io_meta_pb2 import OutlierMeta
from kernel.protobuf.generated.data_io_param_pb2 import DataIOParam
from kernel.protobuf.generated.data_io_param_pb2 import ImputerParam
from kernel.protobuf.generated.data_io_param_pb2 import OutlierParam
...
class DenseFeatureReader(object):
    def __init__(self, data_io_param):
        ...
        self.missing_fill_rule = data_io_param.missing_fill_rule
        self.need_features = data_io_param.need_features
        self.need_features_index = None
    ...
    def process_value(self, value, data_shape=0):
        if not value:
            return []
        if self.with_label and data_shape == 1:
            return []
        value_list = value.split(self.delimitor, -1)
        if self.with_label and self.label_idx >= 0 and self.label_idx < len(value_list):
            value_list.pop(self.label_idx)
        if self.need_features_index:
            return [value_list[i] for i in self.need_features_index]
        else:
            return value_list
    ...
    def read_data(self, input_data, mode="fit"):
        ...
        print("input_data first type", type(input_data.first()))
        print("input_data first", input_data.first())

        # print("input_data ", list(input_data.collect()))
        # print("input_data count", input_data.count())
        # input_data
        # [('0', '0,1.886690,-1.359293,2.303601,2.001237,1.307686,2.616665,2.109526,2.296076,2.750622,1.937015'),
        # ('1', '0,1.805927,-0.369203,1.535126,1.890489,-0.375612,-0.430444,-0.146749,1.087084,-0.243890,0.281190'),

        data_util.empty_table_detection(input_data)
        ...
        if self.with_label:
            data_shape = data_util.get_data_shape(input_data)
            ...
            input_data_features = input_data.mapValues(
                lambda value: self.process_value(value, data_shape))
            input_data_labels = input_data.mapValues(lambda value: value.split(self.delimitor, -1)[self.label_idx])

        else:
            input_data_features = input_data.mapValues(
                lambda value: self.process_value(value))
            self.label_name = ""

        # print("input_data_features", list(input_data_features.collect()))
        # input_data_features
        ...
        # print("input_data_labels", list(input_data_labels.collect()))
        # input_data_labels [('0', '0'), ('1', '0'), ('2', '0'), ('3', '0'),
        # print("input_data_features[1]", input_data_features.first())
        # print("input_data_labels[1]", input_data_labels.first())
        ...
        # print("data_instance", list(data_instance.collect()))
        # print("data_instance count", data_instance.count())
        # data_instance [('0', <kernel.feature.instance.Instance object at 0x1122a4b38>), ('1', <kernel.feature.instance.Instance object at 0x1f221aaac8>),
        print("data_instance[1]", data_instance.first()[1].features)
        print("data_instance:", data_instance.schema)
        # data_instance[1] [ 1.88669  -1.359293  2.303601  2.001237  1.307686  2.616665  2.109526 2.296076  2.750622  1.937015]
    ...
    def fill_missing_value(self, input_data_features, mode="fit"):
        if self.missing_fill:
            # from kernel.feature.imputer import Imputer
            # imputer_processor = Imputer(self.missing_impute)
            from kernel.base.imputer_missing_fill import ImputerMissingFill
            imputer_processor = ImputerMissingFill(self.missing_impute, self.missing_fill_rule)
            if mode == "fit":
        ...
        if output_format == "dense":
            if data_type in ["int", "int64", "long", "float", "float64", "double"]:
                for i in range(len(features)):
                    if (missing_impute is not None and features[i] in missing_impute) or \
                            (missing_impute is None and features[i] in ['', 'NULL', 'null', "NA", "N/A"]):
                        features[i] = np.nan
            ...
            if exclusive_data_type_fid_map:
                ...
                for feature in features:
                    if DenseFeatureReader.is_number(feature):
                        new_features.append(float(feature))
                    else:
                        new_features.append(feature)
                return np.asarray(new_features, dtype=object)
...
class SparseFeatureReader(object):
    ...
    def read_data(self, input_data, mode="fit"):
        ...
        if not data_util.get_data_shape(input_data):
            raise ValueError("input data's value is empty, it does not contain a label")
...
class DataIO(ModelBase):
    def __init__(self):
        ...
        self.reader = None
        from kernel.components.dataio.param import DataIOParam
