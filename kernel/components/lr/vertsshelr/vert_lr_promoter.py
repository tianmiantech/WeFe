...
from kernel.utils import LOGGER, consts, custom_operator
from kernel.utils.io_check import assert_io_num_rows_equal


class VertLRPromoter(VertLRBase):
    ...
    def predict(self, data_instances):
        ...
        def _vec_dot(v, coef, intercept):
            return custom_operator.vec_dot(v.features, coef) + intercept
    ...
    def load_single_model(self, single_model_obj):
        super(VertLRPromoter, self).load_single_model(single_model_obj)
        if not self.is_respectively_reveal:
    ...
    def get_model_summary(self):
        summary = super(VertLRPromoter, self).get_model_summary()
        return summary
