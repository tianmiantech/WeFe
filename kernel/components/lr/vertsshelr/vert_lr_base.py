...
from kernel.security.protol.spdz.tensor import fixedpoint_numpy, fixedpoint_table
from kernel.transfer.framework.vert.procedure import batch_generator
from kernel.transfer.variables.transfer_class.batch_generator_transfer_variable import BatchGeneratorTransferVariable
from kernel.transfer.variables.transfer_class.converge_checkerk_transfer_variable import ConvergeCheckerTransferVariable
from kernel.transfer.variables.transfer_class.sshe_model_transfer_variable import SSHEModelTransferVariable
from kernel.utils import consts, LOGGER


class VertLRBase(BaseLRModel, ABC):
    def __init__(self):
        super().__init__()
        self.model_name = 'VertSSHELogisticRegression'
        self.model_param_name = 'VertSSHELogisticRegressionParam'
        self.model_meta_name = 'VertSSHELogisticRegressionMeta'
    ...
    def _init_model(self, params: LogisticParam):
        ...
        self.batch_generator = batch_generator.Promoter() if self.role == consts.PROMOTER else batch_generator.Provider()
                ...
                for batch_idx, batch_data in enumerate(encoded_batch_data):
                    ...
                    if self.reveal_every_iter:
                        ...
                        if new_g is not None:
                            ...
                            self.model_weights = LRModelWeights(w=np.zeros(self_g.shape),
                                                                fit_intercept=self.model_param.init_param.fit_intercept)
                    else:
