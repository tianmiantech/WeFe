...
from kernel.utils.horz_label_encoder import HorzLabelEncoderClient
...
class TableDataSet(DatasetMixIn):
    ...
    def __init__(
        ...
        # partition
        self.partitions = data_instances._partitions
        ...
        if label_align_mapping is None:
            ...
            _, label_align_mapping = HorzLabelEncoderClient().label_alignment(labels)
...
class VisionDataSet(torchvision.datasets.VisionDataset, DatasetMixIn):
    ...
    def __init__(self, root, is_train=True, expected_label_type=np.float32, **kwargs):
        ...
        # fake alignment
        if is_train:
            HorzLabelEncoderClient().label_alignment(["fake"])
