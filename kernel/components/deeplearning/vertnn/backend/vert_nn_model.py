...
from kernel.protobuf.generated.vert_nn_model_param_pb2 import VertNNModelParam

LOGGER = log_utils.get_logger()


class VertNNKerasPromoterModel(VertNNPromoterModel):
    def __init__(self, vert_nn_param):
        super(VertNNKerasPromoterModel, self).__init__()
        ...
        self.selector = SelectorFactory.get_selector(vert_nn_param.selector_param.method,
                                                     vert_nn_param.selector_param.selective_size,
                                                     beta=vert_nn_param.selector_param.beta,
                                                     random_rate=vert_nn_param.selector_param.random_state,
                                                     min_prob=vert_nn_param.selector_param.min_prob)

    def set_nn_meta(self, vert_nn_param):
        self.bottom_nn_define = vert_nn_param.bottom_nn_define
        self.top_nn_define = vert_nn_param.top_nn_define
        self.interactive_layer_define = vert_nn_param.interactive_layer_define
        self.config_type = vert_nn_param.config_type
        self.optimizer = vert_nn_param.optimizer
        self.loss = vert_nn_param.loss
    ...
    def train(self, x, y, epoch, batch_idx):
        ...
        if not self.is_empty:
            ...
            promoter_bottom_output = self.bottom_model.forward(x)
        else:
            promoter_bottom_output = None

        if self.interactive_model is None:
            self._build_interactive_model()

        interactive_output = self.interactive_model.forward(promoter_bottom_output, epoch, batch_idx, train=True)
    ...
    def predict(self, x):
        ...
        interactive_output = self.interactive_model.forward(promoter_bottom_output, train=False)
    ...
    def evaluate(self, x, y, epoch, batch):
        ...
        interactive_output = self.interactive_model.forward(promoter_bottom_output, epoch, batch, train=False)
        metrics = self.top_model.evaluate(interactive_output, y)

        return metrics

    def get_vert_nn_model_param(self):
        model_param = VertNNModelParam()
    ...
    def get_vert_nn_model_meta(self):
        ...
        model_meta.interactive_layer_lr = self.vert_nn_param.interactive_layer_lr
    ...
    def _build_bottom_model(self):
        self.bottom_model = VertNNBottomModel(input_shape=self.bottom_model_input_shape,
    ...
    def _build_top_model(self):
        self.top_model = VertNNTopModel(input_shape=self.top_model_input_shape,
                                        optimizer=self.optimizer,
    ...
    def _build_interactive_model(self):
        self.interactive_model = InterActivePromoterDenseLayer(self.vert_nn_param,
...
class VertNNKerasHostModel(VertNNHostModel):
    ...
    def predict(self, x):
        promoter_bottom_output = self.bottom_model.predict(x)
        self.interactive_model.forward(promoter_bottom_output, train=False)

    def evaluate(self, x, epoch, batch_idx):
        promoter_bottom_output = self.bottom_model.predict(x)
        self.interactive_model.forward(promoter_bottom_output, epoch, batch_idx, train=False)
