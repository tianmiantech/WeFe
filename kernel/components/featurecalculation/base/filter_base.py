...
class BaseFilterMethod(object):
    ...
    def __init__(self, filter_param):
        self.calculation_properties: CalculationProperties = None
        self._parse_filter_param(filter_param)

    @property
    def feature_values(self):
        return self.calculation_properties.feature_values
    ...
    def set_calculation_properties(self, calculation_properties):
    ...
    def _keep_one_feature(self, pick_high=True):
        ...
        """
        if len(self.calculation_properties.left_col_indexes) > 0:
            return

        LOGGER.info("All features has been filtered, keep one without satisfying all the conditions")

        LOGGER.debug("feature values: {}, calculate_col_names: {}, left_col_names: {}".format(
            self.feature_values, self.calculation_properties.calculate_col_names,
            self.calculation_properties.left_col_names
        ))

        # random pick one
        if len(self.feature_values) == 0:
            left_col_name = random.choice(self.calculation_properties.calculate_col_names)
        else:
        ...
        self.calculation_properties.add_left_col_name(left_col_name)
