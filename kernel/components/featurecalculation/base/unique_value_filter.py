...
from kernel.components.featurecalculation.param import UniqueValueParam
...
class UniqueValueFilter(BaseFilterMethod):
    ...
    def fit(self, data_instances, suffix):
        ...
        for col_name in self.calculation_properties.calculate_col_names:
            ...
            self.calculation_properties.add_feature_value(col_name, min_max_diff)
        self._keep_one_feature(pick_high=True)
        return self

    def get_meta_obj(self, meta_dicts):
        result = feature_calculation_meta_pb2.UniqueValueMeta(eps=self.eps)
