...
class IVValueCalculationParam(BaseParam):
    """
    Use information values to calculat features.

    Parameters
    ----------
    value_threshold: float, default: 1.0
        Used if iv_value_thres method is used in feature calculation.

    provider_thresholds: List of float or None, default: None
        Set threshold for different provider. If None, use same threshold as promoter. If provided, the order should map with
        the provider id setting.

    """

    def __init__(self, value_threshold=0.0, provider_thresholds=None, local_only=False):
        super().__init__()
        self.value_threshold = value_threshold
        self.provider_thresholds = provider_thresholds
        self.local_only = local_only

    def check(self):
        if not isinstance(self.value_threshold, (float, int)):
            raise ValueError("IV calculation param's value_threshold should be float or int")

        if self.provider_thresholds is not None:
            if not isinstance(self.provider_thresholds, list):
                raise ValueError("IV calculation param's host_threshold should be list or None")

        if not isinstance(self.local_only, bool):
            raise ValueError("IV calculation param's local_only should be bool")

        return True


class IVPercentileCalculationParam(BaseParam):
    """
    Use information values to calculat features.
    ...
    def check(self):
        descr = "IV calculation param's"
        self.check_decimal_float(self.percentile_threshold, descr)
        self.check_boolean(self.local_only, descr)
        return True


class VarianceOfCoeCalculationParam(BaseParam):
    """
    Use coefficient of variation to calculat features. When judging, the absolute value will be used.

    Parameters
    ----------
    value_threshold: float, default: 1.0
        Used if coefficient_of_variation_value_thres method is used in feature calculation. Filter those
...
class OutlierColsCalculationParam(BaseParam):
    """
...
class FeatureCalculationParam(BaseParam):
    """
    Define the feature calculation parameters.

    Parameters
    ----------
    calculate_col_indexes: list or int, default: -1
        Specify which columns need to calculated. -1 represent for all columns.

    calculate_names : list of string, default: []
    ...
    filter_methods: list, ["manually", "unique_value", "iv_value_thres", "iv_percentile",
        ...
        Specify the filter methods used in feature calculation. The orders of filter used is depended on this list.
    ...
    def __init__(self, calculate_col_indexes=-1, calculate_names=None, filter_methods=None,
                 unique_param=UniqueValueParam(),
                 iv_value_param=IVValueCalculationParam(),
                 iv_percentile_param=IVPercentileCalculationParam(),
                 variance_coe_param=VarianceOfCoeCalculationParam(),
                 outlier_param=OutlierColsCalculationParam(),
                 manually_param=ManuallyFilterParam(),
                 need_run=True
                 ):
        super(FeatureCalculationParam, self).__init__()
        self.calculate_col_indexes = calculate_col_indexes
        if calculate_names is None:
            self.calculate_names = []
        else:
            self.calculate_names = calculate_names
    ...
    def check(self):
        descr = "vert feature calculation param's"
        ...
        self.check_defined_type(self.calculate_col_indexes, descr, ['list', 'int'])
