...
from kernel.components.feature.statistic.core.base_statistic import calc_label_statistic, calc_local_statistic, \
    transfer_magnitude_names, generate_statistic_result, calc_magnitude_result
...
class MixStatisticPromoter(MixStatisticBase):
    def __init__(self):
        super(MixStatisticPromoter, self).__init__()
    ...
    def fit(self, data_instances):
        ...
        member_statistics = [{"member_id": self.member_id, "role": self.role, "feature_statistic": feature_statistic,
                              'label_statistic': label_statistic}]
        provider_statistic_results = self.transfer_variable.provider_statistic_result.get(-1)
        for provider_statistic_results in provider_statistic_results:
            member_statistics.append(provider_statistic_results)

        feature_statistics = {'members': member_statistics}
        LOGGER.debug(f'feature_statistics={feature_statistics}')
