...
from kernel.security.cipher_compressor.compressor import CipherCompressor, NormalCipherPackage, CipherDecompressor
...
class PromoterGradHessEncoder(object):
...
class PromoterSplitInfoDecompressor(object):
...
class ProviderSplitInfoCompressor(object):
    ...
    def renew_compressor(self, node_sample_count, node_map):
        for node_id, idx in node_map.items():
            ...
            self.compressors[node_id] = CipherCompressor(self.encrypt_type, max_capacity_int=self.max_capacity_int,
                                                         ...
                                                         max_float=max_float)
            _, capacity = CipherCompressor.advise(max_float, self.max_capacity_int, self.encrypt_type,
                                                  self.round_decimal)
...
if __name__ == '__main__':
    ...
    import numpy as np
    from kernel.security import PaillierEncrypt as Encrypt
    from kernel.security import IterativeAffineEncrypt as Encrypt
    ...
    encoder = PromoterGradHessEncoder(en, None, )
    compressor = ProviderSplitInfoCompressor(key_length, consts.ITERATIVEAFFINE, )
    decompressor = PromoterSplitInfoDecompressor(en, )
