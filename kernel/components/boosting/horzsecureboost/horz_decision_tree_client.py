...
from kernel.components.boosting import Splitter
from kernel.components.boosting.param import DecisionTreeParam
...
from kernel.transfer.variables.transfer_class.horz_decision_tree_transfer_variable import \
    HorzDecisionTreeTransferVariable
...
class HorzDecisionTreeClient(DecisionTree):

    def __init__(self, tree_param: DecisionTreeParam, data_bin=None, bin_split_points: np.array = None,
                 bin_sparse_point=None, g_h=None, valid_feature: list = None, epoch_idx: int = None,
                 role: str = None, tree_idx: int = None, flow_id: str = None, mode='train'):
        ...
        role: provider or promoter
        flow_id: flow id
        mode: train / predict
        """

        super(HorzDecisionTreeClient, self).__init__(tree_param)
        ...
        self.transfer_inst = HorzDecisionTreeTransferVariable()
    ...
    def fit(self):
        ...
        start to fit
        """
        LOGGER.info('begin to fit horz decision tree, epoch {}, tree idx {}'.format(self.epoch_idx, self.tree_idx))
        ...
        # initialize node
        root_node = Node(id=0, sitename=consts.PROMOTER, sum_grad=global_g_sum, sum_hess=global_h_sum, weight=
        self.splitter.node_weight(global_g_sum, global_h_sum))

        self.cur_layer_node = [root_node]
        LOGGER.debug('assign samples to root node')
        self.node_dispatch = self.assign_instance_to_root_node(self.data_bin, 0)
        # print(list(self.node_dispatch.collect()))
        for dep in range(self.max_depth):
            ...
            if dep + 1 == self.max_depth:
                ...
                rest_sample_weights = self.get_node_sample_weights(self.node_dispatch, self.tree_node)
            ...
            table_with_assignment = self.data_bin.join(self.node_dispatch, lambda inst, assignment: (inst, assignment))
            print(table_with_assignment.first())
            ...
            self.node_dispatch, leaf_val = self.assign_instance_to_new_node(table_with_assignment, self.tree_node)
