...
from kernel.components.boosting.mixsecureboost.mix_decision_tree_promoter import MixDecisionTreePromoter
...
from kernel.utils.data_util import separate
...
class MixSecureBoostingPromoter(BoostingTree):
    def __init__(self):
        super(MixSecureBoostingPromoter, self).__init__()
    ...
    def _init_model(self, param: MixSecureBoostParam):
        ...
        super(MixSecureBoostingPromoter, self)._init_model(param)
    ...
    def convert_feature_to_bin(self, data_instance):
        ...
        provider_split_points = self.get_provider_split_points()
        l = len(split_points)
        tmp, k_dict = {}, {}
        size_list = [l]
        # concat all provider split points
        for i,provider_split_point in enumerate(provider_split_points):
            size_list.append(len(provider_split_point))
            for k in provider_split_point.keys():
                k_n = str(i) + k
                tmp[k_n] = provider_split_point[k]
                k_dict[k_n] = k
                # l += 1
        split_points.update(tmp)

        # aggregate split points
        split_points = {k: np.array(v) for k, v in split_points.items()}
        split_points_weights = DictWeights(d=split_points)
        self.binning_obj.split_points_aggregator.send_model(split_points_weights)
        dict_split_points = self.binning_obj.split_points_aggregator.get_aggregated_model()
        split_points = {k: list(v) for k, v in dict_split_points.unboxed.items()}

        # separate and send split points
        split_points_list = separate(list(split_points.items()), size_list)
        promoter_split_points = dict(split_points_list[0])
        final_split_points_list = []
        for sp in split_points_list[1:]:
            sp_dict = {k_dict[k]: v for k, v in dict(sp).items()}
            final_split_points_list.append(sp_dict)
        self.send_optim_split_points(final_split_points_list)

        self.data_bin, self.bin_split_points, self.bin_sparse_points = binning_obj.convert_feature_to_bin(data_instance,
                                                                                                          promoter_split_points)
    ...
    def sync_provider_feature_num(self):
        return self.transfer_variable.provider_feature_number.get(idx=-1, suffix=('feat_num',))

    def get_provider_split_points(self):
        return self.transfer_variable.provider_split_points.get(idx=-1)

    def send_optim_split_points(self, split_points_list):
        for idx, split_info in enumerate(split_points_list):
            self.transfer_variable.optim_split_points.remote(split_info,
                                                             role=consts.PROVIDER,
                                                             idx=idx)

    def sync_feature_num(self, feature_num=None):
        self.transfer_variable.feature_number.remote(feature_num, role=consts.ARBITER, idx=-1, suffix=('feat_num',))
    ...
    def sync_stop_flag(self, stop_flag, num_round):
        ...
        self.transfer_variable.provider_stop_flag.remote(stop_flag,
                                                         role=consts.PROVIDER,
                                                         idx=-1,
                                                         suffix=(num_round,))
    ...
    def load_booster(self, model_meta, model_param, epoch_idx, booster_idx):
        tree = MixDecisionTreePromoter(self.tree_param)
    ...
    @staticmethod
    def traverse_a_tree(tree: MixDecisionTreePromoter, sample, cur_node_idx):
    ...
    @staticmethod
    def traverse_trees(node_pos, sample, trees: List[MixDecisionTreePromoter]):
        ...
        for t_idx, tree in enumerate(trees):
            ...
            rs, reach_leaf = MixSecureBoostingPromoter.traverse_a_tree(tree, sample, cur_node_idx)
    ...
    @staticmethod
    def add_y_hat(leaf_pos, init_score, learning_rate, trees: List[MixDecisionTreePromoter], multi_class_num=None):
    ...
    @staticmethod
    def get_predict_scores(leaf_pos, learning_rate, init_score, trees: List[MixDecisionTreePromoter]
                           , multi_class_num=-1):

        predict_func = functools.partial(MixSecureBoostingPromoter.add_y_hat,
    ...
    def fit_a_booster(self, epoch_idx: int, tree_dim: int):

        g_h = self.get_grad_and_hess(tree_dim)

        tree = MixDecisionTreePromoter(tree_param=self.tree_param)
        tree.init(flowid=self.generate_flowid(epoch_idx, tree_dim),
    ...
    def fit(self, data_inst, validate_data=None):
        ...
        self.feature_num = self.bin_split_points.shape[0]
        # sync feature num
        provider_feature_nums = self.sync_provider_feature_num()
        feature_num = self.feature_num
        for num in provider_feature_nums:
            feature_num += num
        self.sync_feature_num(feature_num=feature_num)
    ...
    def boosting_fast_predict(self, data_inst, trees: List[MixDecisionTreePromoter]):
    ...
    def predict(self, data_inst):
        ...
        predict_rs = self.boosting_fast_predict(processed_data, trees=trees)
    ...
    def get_model_meta(self):
        ...
        meta_name = "MixSecureBoostingTreePromoterMeta"
    ...
    def get_model_param(self):
        ...
        param_name = "MixSecureBoostingTreePromoterParam"
