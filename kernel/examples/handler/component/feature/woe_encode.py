...
class WoeEncodeParameter(WoeEncodeParam):
...
class WoeEncode(Component, WoeEncodeParameter):
    def __init__(self, **kwargs):
        ...
        WoeEncodeParameter.__init__(self, **new_kwargs)

        self.input = Input(self.name)
        self.output = Output(self.name, data_type='single')
        self._module_name = "Woe"
        self._param_name = "WoeEncodeParam"
