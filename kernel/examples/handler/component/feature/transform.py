...
class TransformParameter(FeatureTransformParam):
...
class Transform(Component, TransformParameter):
    def __init__(self, **kwargs):
        ...
        TransformParameter.__init__(self, **new_kwargs)

        self.input = Input(self.name)
        self.output = Output(self.name, data_type='single')
        self._module_name = "FeatureTransform"
        self._param_name = "FeatureTransformParam"
