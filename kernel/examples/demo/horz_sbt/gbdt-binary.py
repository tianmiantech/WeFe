...
def main(config="../../config.yaml", param="./binary_config.yaml"):
    ...
    clf = GradientBoostingClassifier(n_estimators=120 if 'epsilon' in data_promoter else 50, learning_rate=0.1)
    clf.fit(X, y)
    y_prob = clf.predict(X_promoter)

    try:
        auc_score = roc_auc_score(y_promoter, y_prob)
    except:
