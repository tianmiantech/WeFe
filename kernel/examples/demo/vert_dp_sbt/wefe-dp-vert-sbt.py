...
from kernel.examples.handler.component import VertDPSecureBoost
...
def main(config="../../config.yaml", param="./binary_config.yaml", namespace="wefe_data"):
    ...
    # init handler
    handler = Handler(job_id="job_dp-vertsbt_"+ str(random.randint(0, 999999999999)), backend=backend, work_mode=work_mode, db_type=db_type,
                      fl_type='vertical') \
    ...
    dp_secure_boost_0 = VertDPSecureBoost(name="vert-dp_secure_boost_0",
    ...
    handler.add_component(dp_secure_boost_0, data=Data(train_data=intersect_0.name))
    handler.add_component(evaluation_0, data=Data(data=dp_secure_boost_0.name))

    handler.compile()
    handler.fit()

    print(handler.get_metric_summary(name='evaluation_0', component_name="vert-dp_secure_boost_0"))
