...
from kernel.examples.handler.component import VertNN
...
def main(config="../../config.yaml", param="./param_conf_binary.yaml", namespace="wefe_data"):
    ...
    # initialize handler
    handler = Handler(job_id="job_vertnn_001", work_mode=work_mode, backend=backend, db_type=db_type,
                      fl_type="horizontal")
    # set job initiator
    ...
    handler.add_component(vert_nn_0, data=Data(train_data=dataio_0.name))
    handler.add_component(evaluation_0, data=Data(data=vert_nn_0.name))
    ...
    print(handler.get_metric_summary(name='evaluation_0', component_name="vert_nn_0"))
