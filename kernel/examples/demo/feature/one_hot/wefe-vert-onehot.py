...
from kernel.examples.handler.component import VertOneHot
...
def main(config="../../config.yaml", param="./onehot_config.yaml", namespace="wefe_data"):
    # obtain config
    ...
    vert_onehot_0 = VertOneHot(name="vert_onehot_0", need_run=True)

    # add components to handler, in order of task execution
    handler.add_component(dataio_0)
    handler.add_component(vert_onehot_0, data=Data(data=dataio_0.name), output_data_type=["train"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("VERT ONE HOT JOB")
    parser.add_argument("-c", "--config", type=str,
                        help="config file", default="../../../config.yaml")
    parser.add_argument("-p", "--param", type=str,
                        help="config file for params", default="./onehot_config.yaml")
