...
from kernel.examples.handler.component import VertFilter
...
def main(config="../../config.yaml", param="./config.yaml", namespace="wefe_data"):
    ...
    vert_sample_filter_params = {
        "filter_rules": "x1>0&x1<10&x2>0.5"
    }
    vert_sample_filter_0 = VertFilter(name="vert_sample_filter_0", **vert_sample_filter_params)

    # add components to handler, in order of task execution
    handler.add_component(dataio_0)
    handler.add_component(vert_sample_filter_0, data=Data(data=dataio_0.name), output_data_type=["train"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Vert Sample Filter JOB")
