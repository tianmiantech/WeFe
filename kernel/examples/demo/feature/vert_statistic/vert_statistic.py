...
from kernel.examples.handler.component import VertStatistic, DataIO
...
def main(config="../../config.yaml", param="./config.yaml", namespace="wefe_data"):
    ...
    vert_statistic_params = {
        "percentage_list": [50]
    }
    vert_statistic_0 = VertStatistic(name="vert_statistic_0", **vert_statistic_params)

    # add components to handler, in order of task execution
    handler.add_component(dataio_0)
    handler.add_component(vert_statistic_0, data=Data(data=dataio_0.name), output_data_type=["train"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Vert Statistic JOB")
