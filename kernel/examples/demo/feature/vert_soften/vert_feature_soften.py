...
from kernel.examples.handler.component import VertSoften
...
def main(config="../../config.yaml", param="./config.yaml", namespace="wefe_data"):
    ...
    vert_feature_soften_params = {
        "soften_rules": "{\"x1\":{\"method\":\"z_score\",\"min\":0.95,\"max\":0},\"x2\":{\"method\":\"min_max_per\",\"min\":0,\"max\":10},\"x3\":{\"method\":\"min_max_thresh\",\"min\":0,\"max\":10}}"
    }
    vert_feature_soften_0 = VertSoften(name="vert_feature_soften_0", **vert_feature_soften_params)

    # add components to handler, in order of task execution
    handler.add_component(dataio_0)
    handler.add_component(vert_feature_soften_0, data=Data(data=dataio_0.name), output_data_type=["train"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("VERT FEATURE SOFTEN JOB")
