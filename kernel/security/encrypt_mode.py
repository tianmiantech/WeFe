...
class EncryptModeCalculator(object):
    ...
    def encrypt(self, input_data):
        ...
        Returns 
        ------- 
        new_data: DSource, encrypted result of input_data

        """
        if self.mode == "strict":
            new_data = input_data.mapValues(self.encrypter.recursive_encrypt, need_send=True)
            return new_data
        else:
            if self.enc_zeros is None or (
                    self.mode == "balance" and self.should_re_encrypted()) \
                    or self.enc_zeros.count() != input_data.count():
                self.enc_zeros = input_data.mapValues(lambda val: self.encrypter.encrypt(0), need_send=True)

            new_data = input_data.join(self.enc_zeros, self.add_enc_zero, need_send=True)
            return new_data
