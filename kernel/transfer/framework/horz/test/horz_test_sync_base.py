...
from kernel.transfer.variables.transfer_class.horz_transfer_variable import HorzTransferVariable
...
class TestSyncBase(unittest.TestCase):
    ...
    def init_table_manager_and_federation(cls, job_id, role, num_hosts, host_ind=0):
        ...
        federation.init(job_id,
                        {"local": {
                            "role": role,
                            "member_id": role_id[role][0] if role != "provider" else role_id[role][host_ind]
                        },
                            "role": role_id
                        })
    ...
    def setUp(self) -> None:
        self.transfer_variable = HorzTransferVariable()
    ...
    def results(cls, job_id, transfer_variable, num_hosts, *args):
        ...
        with Pool(num_hosts + 2) as p:
            ...
            tasks.append(p.apply_async(func=cls._call,
                                       args=(job_id, consts.PROMOTER, transfer_variable, num_hosts, 0, *args)))
            for i in range(num_hosts):
                tasks.append(
                    p.apply_async(func=cls._call,
                                  args=(job_id, consts.PROVIDER, transfer_variable, num_hosts, i, *args)))
