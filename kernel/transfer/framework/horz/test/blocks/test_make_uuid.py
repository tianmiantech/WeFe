...
from kernel.transfer.framework.horz.blocks import uuid_generator
...
class IdentifyUUIDTest(TestBlocks):
    ...
    def run_uuid_test(self, num_hosts):
        uuid_set, promoter_uuid, hosts_uuid = self.run_test(uuid_call, self.job_id, num_hosts=num_hosts)
        self.assertEqual(len(hosts_uuid), num_hosts)
        self.assertIn(promoter_uuid, uuid_set)
