...
from kernel.transfer.framework.horz.blocks import has_converged
...
class ModelBroadcasterTest(TestBlocks):
    ...
    def run_with_num_hosts(self, num_hosts):
        ...
        arbiter, promoter, hosts = self.run_test(model_broadcaster_call, self.job_id, num_hosts, status)
        self.assertEqual(promoter, status)
