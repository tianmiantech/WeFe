...
from kernel.transfer.framework.horz.sync import is_converge_sync
...
class IsConvergeTest(TestSyncBase):
    ...
    def call(cls, role, transfer_variable, ind, *args):
        ...
        if role == consts.ARBITER:
            ...
            status = []
            is_converge = is_converge_sync.Provider().register_is_converge(transfer_variable.is_converge)
            for i in range(2):
                for j in range(2):
            ...
            status = []
            is_converge = is_converge_sync.Promoter().register_is_converge(transfer_variable.is_converge)
            for i in range(2):
                for j in range(2):
                    status.append(is_converge.get_converge_status(suffix=(i, j)))
            return status
    ...
    def run_with_num_hosts(self, num_hosts):
        arbiter, promoter, *hosts = self.run_results(num_hosts=num_hosts)
        self.assertListEqual(arbiter, promoter)
        for state in hosts:
