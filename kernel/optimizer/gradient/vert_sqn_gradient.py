...
from kernel.optimizer.gradient import sqn_sync
from kernel.utils import consts

LOGGER = log_utils.get_logger()


class VertStochasticQuansiNewton(vert_lr_gradient_and_loss.VertGradientBase):
    def __init__(self, sqn_param: StochasticQuasiNewtonParam):
        ...
        self.batch_index = 0
        self.last_w_tilde: LRModelWeights = None
        self.this_w_tilde: LRModelWeights = None
    ...
    def _renew_w_tilde(self):
        ...
        self.this_w_tilde = LRModelWeights(np.zeros_like(self.last_w_tilde.unboxed),
    ...
    def federated_compute_gradient(self, *args):
        ...
        gradient_results = self.gradient_computer.federated_compute_gradient(*args)
...
class VertStochasticQuansiNewtonPromoter(VertStochasticQuansiNewton):
    def __init__(self, sqn_param):
        super().__init__(sqn_param)
        self.sqn_sync = sqn_sync.Promoter()
    ...
    def _update_hessian(self, data_instances, optimizer, cipher_operator):
        ...
        provider_forwards = self.sqn_sync.get_provider_forwards(suffix=suffix)
        forward_hess, hess_vector = self.gradient_computer.compute_forward_hess(sampled_data, delta_s,
                                                                                provider_forwards)
...
class VertStochasticQuansiNewtonProvider(VertStochasticQuansiNewton):
    def __init__(self, sqn_param):
        super().__init__(sqn_param)
        self.sqn_sync = sqn_sync.Provider()
    ...
    def _update_hessian(self, data_instances, optimizer, cipher_operator):
        ...
        provider_forwards = self.gradient_computer.compute_sqn_forwards(sampled_data, delta_s, cipher_operator)
        # provider_forwards = cipher_operator.encrypt_list(provider_forwards)
        self.sqn_sync.remote_provider_forwards(provider_forwards, suffix=suffix)
...
class VertStochasticQuansiNewtonArbiter(VertStochasticQuansiNewton):
    def __init__(self, sqn_param):
        super().__init__(sqn_param)
        self.opt_Hess = None
        self.opt_v = None
        ...
        self.model_weight: LRModelWeights = None

    def _update_w_tilde(self, gradient: LRModelWeights):
    ...
    def federated_compute_gradient(self, cipher_operator, optimizer, n_iter_, batch_index):
        ...
        delta_grad = self.gradient_computer.federated_compute_gradient(
            cipher_operator, optimizer, n_iter_, batch_index)
        self._update_w_tilde(LRModelWeights(delta_grad, fit_intercept=False))
        if self.iter_k % self.update_interval_L == 0:
            self.count_t += 1
            LOGGER.debug("Before division, this_w_tilde: {}".format(self.this_w_tilde.unboxed))
            self.this_w_tilde /= self.update_interval_L
    ...
    def _compute_hess_matrix(self):
        ...
        for y, s in zip(self.opt_v, self.opt_s):
            ...
            Hess = (np.identity(n) - rho * s.dot(y.T)).dot(Hess).dot(np.identity(n) - rho * y.dot(s.T)) + rho * s.dot(
                s.T)
            iter_num += 1
            LOGGER.info("hessian updating algorithm iter_num = {}, rho = {} \n ||s|| is {} \n ||y|| is {}"
                        .format(iter_num, rho, np.linalg.norm(s), np.linalg.norm(y)))
...
def sqn_factory(role, sqn_param):
    if role == consts.PROMOTER:
        return VertStochasticQuansiNewtonPromoter(sqn_param)

    if role == consts.PROVIDER:
        return VertStochasticQuansiNewtonProvider(sqn_param)

    return VertStochasticQuansiNewtonArbiter(sqn_param)
