...
class _Optimizer(object):
    ...
    def _l1_updator(self, model_weights: LRModelWeights, gradient):
    ...
    def regularization_update(self, model_weights: LRModelWeights, grad):
        if self.penalty == consts.L1_PENALTY:
            ...
            model_weights = LRModelWeights(new_vars, model_weights.fit_intercept)
        return model_weights

    def __l1_loss_norm(self, model_weights: LRModelWeights):
        coef_ = model_weights.coef_
        loss_norm = np.sum(self.alpha * np.abs(coef_))
        return loss_norm

    def __l2_loss_norm(self, model_weights: LRModelWeights):
        coef_ = model_weights.coef_
        loss_norm = 0.5 * self.alpha * np.dot(coef_, coef_)
        return loss_norm

    def loss_norm(self, model_weights: LRModelWeights):
        if self.penalty == consts.L1_PENALTY:
    ...
    def hess_vector_norm(self, delta_s: LRModelWeights):
        if self.penalty == consts.L1_PENALTY:
            return LRModelWeights(np.zeros_like(delta_s.unboxed), fit_intercept=delta_s.fit_intercept)
        elif self.penalty == consts.L2_PENALTY:
            return LRModelWeights(self.alpha * np.array(delta_s.unboxed), fit_intercept=delta_s.fit_intercept)
        else:
            return LRModelWeights(np.zeros_like(delta_s.unboxed), fit_intercept=delta_s.fit_intercept)

    def update_model(self, model_weights: LRModelWeights, grad, has_applied=True):
        if not has_applied:
