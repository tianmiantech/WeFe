...
class Imputer(object):
    ...
    def __get_impute_rate_from_replace_data(self, data):
        ...
        impute_number_statics = data.mapPartitions(self.__get_impute_number).reduce(f)
    ...
    def fit(self, data, replace_method=None, replace_value=None, output_format=consts.ORIGIN, quantile=None):
        ...
        ----------
        data: DSource,
            each data's value should be list
        replace_method: str,
            the strategy of imputer, like min, max, mean or designated and so on. Default None
        replace_value: str,
            if replace_method is designated, you should assign the replace_value
            which will be used to replace the value in imputer_value_list
        output_format: str,
            the output data format. The output data can be 'str', 'int', 'float'.
            Default origin, the original format as input data
        ...
        process_data, cols_transform_value = self.fit_replace(data, replace_method, replace_value, output_format,
                                                              quantile=quantile)
    ...
    def transform(self, data, transform_value, output_format=consts.ORIGIN):
        ...
        Parameters
        ----------
        data: DSource, each data's value should be list
        transform_value:
        output_format: str, the output data format. The output data can be 'str', 'int', 'float'.
        Default origin, the original format as input data
