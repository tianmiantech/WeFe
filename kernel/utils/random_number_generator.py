...
class RandomNumberGenerator(object):
    def __init__(self, lower_bound=None, upper_bound=None):
        self.lower_bound = -2 ** BITS if lower_bound is None else lower_bound
        self.upper_bound = 2 ** BITS if upper_bound is None else upper_bound
    ...
    def fast_generate_random_number(self, shape, partition=10, mixed_rate=MIXED_RATE, keep_table=None):
        if keep_table:
            ...
            tb = session.parallelize([None for _ in range(shape[0])], include_key=False, partition=partition)

            tb = tb.mapValues(lambda val: self.generate_random_number(shape[1:], mixed_rate=mixed_rate))

            return PaillierTensor(tb_obj=tb)

    def generate_fake_random_number(self, shape, seed=None):
        random.seed(seed)
        size = self.get_size_by_shape(shape)
        return np.reshape([random.uniform(self.lower_bound, self.upper_bound) for idx in range(size)], shape)
