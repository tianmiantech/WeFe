        ...
        methods: {
                ...
                if(this.unUseParams) {
                    for (const key in query) {
                        if (!this.unUseParams.includes(key)) {
                            if (this.vData) {
                                this.vData.search[key] = query[key] || '';
                            } else {
            ...
            async getList (opt = {
                ...
                for (const field in this.search) {
                    if (!this.unUseParams.includes(field)) {
                        searchFields[field] = this.search[field];
                    }
                }

                const params = {
                    [this.requestMethod === 'get' ? 'params' : 'data']: Object.assign(searchFields, {
            ...
            currentPageChange (val) {
                if (this.watchRoute || this.turnPageRoute) {
                    ...
                    this.pagination.page_index = val || 1;
                    this.getList();
                }
            },
