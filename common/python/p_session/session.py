...
class WefeSession(object):
    _instance: 'WefeSession' = None
    __lock = threading.Lock()

    @staticmethod
    def set_instance(instance):
        if not WefeSession._instance:
            with WefeSession.__lock:
                if not WefeSession._instance:
                    WefeSession._instance = instance

    @staticmethod
    def exit():
        WefeSession._instance = None

    @staticmethod
    def get_instance():
        return WefeSession._instance
    ...
    def get_data_table(name, namespace, partition=1):
        ...
        :return:
            data table instance
        """
        return WefeSession.get_instance().table(name=name,
                                                namespace=namespace,
    ...
    def save_data_table_meta(kv, data_table_name, data_table_namespace):
        ...
        :return:
        """
        from common.python.utils.core_utils import json_dumps
        data_meta_table = WefeSession.get_instance().table(name="%s.meta" % data_table_name,
                                                           ...
                                                           in_place_computing=False)
        for k, v in kv.items():
            data_meta_table.put(k, json_dumps(v), use_serialize=True)
    ...
    def get_data_table_meta(key, data_table_name, data_table_namespace):
        ...
        :return:
        """
        from common.python.utils.core_utils import json_loads
        data_meta_table = WefeSession.get_instance().table(name="%s.meta" % data_table_name,
                                                           namespace=data_table_namespace,
                                                           create_if_missing=True,
                                                           error_if_exist=False,
                                                           ...
                                                           partition=1)
        if data_meta_table:
            value_bytes = data_meta_table.get(key, use_serialize=True)
            if value_bytes:
    ...
    def get_data_table_metas(data_table_name, data_table_namespace):
        ...
        if data_meta_table:
            metas = dict()
            for k, v in data_meta_table.collect(use_serialize=True):
    ...
    def clean_table(namespace, regex_string='*'):
        try:
            WefeSession.get_instance().cleanup(name=regex_string, namespace=namespace, persistent=False)
    ...
    def save_data(kv_data: Iterable,
        ...
        :return:
            data table instance
        """
        from common.python.utils import version_control
        data_table = WefeSession.get_instance().table(name=name,
                                                      namespace=namespace,
                                                      partition=partition,
