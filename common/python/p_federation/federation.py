...
from common.python.common.consts import CONF_KEY_LOCAL, TRANSFER_CONF_PATH, ROLES
from common.python.utils.auth import FederationAuthorization
...
class Federation(object):
    ...
    def __init__(self, session_id, runtime_conf):
        ...
        self._member_id = runtime_conf.get(CONF_KEY_LOCAL).get("member_id")
        self._local_party = Member(self._role, self._member_id)
        self._session_id = session_id
        self._authorize = FederationAuthorization(TRANSFER_CONF_PATH)
        self._role_to_parties_map = {}
        self._all_parties = []
        for role in ROLES:
            member_id_list = runtime_conf.get('role').get(role, [])
    ...
    def role_to_party(self, role, idx) -> Member:
