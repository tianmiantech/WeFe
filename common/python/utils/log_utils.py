...
class LoggerFactory(object):
    ...
    def set_directory(directory=None, parent_log_dir=None, append_to_parent_log=None, force=False):
        ...
        with LoggerFactory.lock:
            if not directory:
                directory = os.path.join(get_log_root_path())
    ...
    def get_handler(class_name, level=None, log_dir=None, log_type=None):
        if not log_type:
            ...
            log_file = os.path.join(log_dir, "wefe_flow_{}.log".format(
                log_type) if level == LoggerFactory.LEVEL else 'error.log')
    ...
    def init_logger(class_name):
        with LoggerFactory.lock:
            ...
            if class_name:
                ...
                sh.setLevel(LoggerFactory.LEVEL)
    ...
    def get_schedule_logger(job_id='', log_type='schedule'):
        job_log_dir = os.path.join(get_log_root_path(), job_id)
        if not job_id:
            job_log_dir = os.path.join(get_log_root_path(), 'wefe_flow')
...
def get_logger(class_name=None, useLevelFile=False):
    if class_name is None:
        class_name = 'stat'
    return LoggerFactory.get_logger(class_name)


def schedule_logger(job_id=None, delete=False):
    if not job_id:
        return get_logger("flow_schedule")
    else:
        if delete:
            with LoggerFactory.lock:
                try:
