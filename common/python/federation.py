...
def init(job_id: str, runtime_conf, server_conf_path="business/conf/server_conf.json"):
    """
    ...
    runtime_conf : dict
      specifiy the role and parties. runtime_conf should be a dict with
        ...
        1. key "local" maps to the current process' role and member_id.
        
        2. key "role" maps to a dict mapping from each role to all involving member_ids.
...
def get(name, tag: str, idx=-1):
    ...
    id x: int
      idx of the member_ids in runtime role list, if idx < 0, list of all objects will be returned.
...
def remote(obj, name: str, tag: str, role=None, idx=-1):
    ...
    idx : int
      The idx of the member_ids of the role, if idx < 0, will send to all parties of the role.
