...
-- author: aaron.li
-- -------------------------------------
ALTER TABLE data_set ADD COLUMN `y_positive_example_count` bigint(20) DEFAULT NULL COMMENT '正例数量';
ALTER TABLE data_set ADD COLUMN `y_positive_example_ratio` double(10,4) DEFAULT NULL COMMENT '正例比例';

alter table operator_log add index log_action_index (log_action);
alter table operator_log add index operator_phone_index (operator_phone);
alter table operator_log add index created_time_index (created_time);
...
-- author: Zane
-- ----------------------------
update project_flow set federated_learning_type='horizontal' where federated_learning_type='0';
update project_flow set federated_learning_type='vertical' where federated_learning_type='1';
update project_flow set federated_learning_type='mix' where federated_learning_type='2';
...
values (replace(uuid(),'-',''), 'member_info', 'member_id', '联邦成员 Id 全局唯一，默认为uuid。', (select member_id from global_setting limit 1)),
