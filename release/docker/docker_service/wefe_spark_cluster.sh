...
stop_python_service(){
    ...
    ssh root@$ip "cd $2 && cd wefe_python_service && sh wefe_python_service_stop.sh"
    sleep 1
}
...
start_python_service(){
    ...
    echo "准备启动远程容器$1, 目前:S2, 身份:$3"
    ssh root@$ip "cd $2 && cd wefe_python_service && sh wefe_python_service_start.sh $3"
    sleep 1
}
