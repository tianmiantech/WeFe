...
# 修改服务启动配置
sed -i "/service_logs/s@-.*:@- \"$DATA_PATH/logs/fusion-service:@g" ./resources/docker-compose.yml
...
# 加载本地离线镜像包
echo "开始加载 fusion-service 离线镜像"
docker load < resources/wefe_fusion_service_${WEFE_VERSION}\.tar
echo "加载 fusion-service 离线镜像完成"
