...
class VarDistributed(object):
    ...
    def __create_var_struct(var):
        return VarStruct(
            var.name, var.shape, var.dtype, var.type, var.lod_level, var.persistable
        )
    ...
    def __str__(self):
        origin_var_str = "{name} : fluid.{type}.shape{shape}.astype({dtype})".format(
            i="{",
            e="}",
...
class VarsDistributed(object):
    ...
    def add_distributed_var(
        ...
        self.distributed_vars.append(
            VarDistributed(
                origin_var, slice_var, is_slice, block_id, offset, vtype, endpoint
            )
        )
